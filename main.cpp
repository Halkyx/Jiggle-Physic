...
class Display
{
    public:
        ...
        void clearScreen()
        {
            SDL_SetRenderDrawColor(ren,178,178,178,256);
    ...
    public:
                ...
                for(int j=0;j<soft_body.size();++j)
                {
                    if(j!=i)//j==i-1||j==i+1||(j==soft_body.size()-1&&i==0)||(i==soft_body.size()-1&&j==0))
                    {
                        ...
                        if(magnitude(d)!=0)
                        {
                            ...
                            force[i].x+=anew.x;
                            force[i].y+=anew.y;
                            force[i].x-=c*magnitude(d)*(velocity[i].x-velocity[j].x)/50.f;
                            force[i].y-=c*magnitude(d)*(velocity[i].y-velocity[j].y)/50.f;
        ...
        void drag(int x, int y)
        {
            ...
            for(int q=0;q<1;++q)
            {
                ...
                d=unit(d);
                velocity[q].x+=5*mouse.x;//+fabs(d.x/10);
                velocity[q].y+=5*mouse.y;//+fabs(d.y/10);
            }
        }
...
{
    Softbody body(0.01,5000,0.05);
    Display display("Jiggle Physic",scr_w,scr_h);
